...
async function cleanData() {
  ...
  const steps = [
    ...
    { label: 'customer_interaction_logs',    fn: () => prisma.customerInteractionLog.deleteMany({ where: { tenant_id: TENANT_ID } }) },
    { label: 'central_info_requests',        fn: () => prisma.centralInfoRequest.deleteMany({ where: { tenant_id: TENANT_ID } }) },
    { label: 'order_forms',                  fn: () => prisma.orderForm.deleteMany({ where: { tenant_id: TENANT_ID } }) },
    ...
    { label: 'message_queue',                fn: () => prisma.messageQueue.deleteMany({ where: { tenant_id: TENANT_ID } }) },
