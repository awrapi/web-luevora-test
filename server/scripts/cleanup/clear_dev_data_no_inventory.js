...
async function main() {
  ...
  deleted.systemGuiderChat       = await prisma.systemGuiderChat.deleteMany().then(r => r.count);
  deleted.systemGuiderTodo       = await prisma.systemGuiderTodo.deleteMany().then(r => r.count);
  ...
  // Media / context caches (PRESERVED FOR INVENTORY)
  ...
  // --- Flush Redis LAST (after DB cleanup) ---
  // Flushing Redis setelah DB agar BullMQ job yang masih running
  // punya waktu untuk mark themselves sebagai 'finished' sebelum key-nya hilang.
  // Jika flush dilakukan duluan, worker akan error: "Missing key for job".
  console.log('\n🔴 Flushing Redis (last step, after DB is clean)...');
  try {
    // Beri jeda 2 detik agar in-flight BullMQ jobs selesai dulu
