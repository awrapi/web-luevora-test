...
const defaultElements = [
  { id: 'doc_title', type: 'text', text: 'INVOICE', x: 50, y: 50, width: 250, height: 50, color: '#1a1a1a', fontSize: 36, fontWeight: 'bold', editable: true, align: 'left' },
  { id: 'invoice_no', type: 'dynamic', text: 'No. Inv: {{NOMOR_INVOICE}}', x: 494, y: 50, width: 250, height: 30, color: '#333333', fontSize: 14, fontWeight: 'bold', editable: false, align: 'right' },
  ...
  { id: 'dates', type: 'dynamic', text: 'Tanggal: {{TANGGAL_TERBIT}}', x: 494, y: 120, width: 250, height: 30, color: '#666666', fontSize: 14, fontWeight: 'normal', editable: false, align: 'right' },
  { id: 'buyer_id', type: 'dynamic', text: 'Kepada:\n{{NAMA_PEMBELI}}\n{{NO_HP_PEMBELI}}', x: 50, y: 220, width: 300, height: 80, color: '#333333', fontSize: 14, fontWeight: 'normal', editable: false, align: 'left' },
  { id: 'packages', type: 'dynamic', text: 'Rincian Paket:\n{{RINCIAN_PAKET}}', x: 50, y: 340, width: 694, height: 100, color: '#333333', fontSize: 14, fontWeight: 'normal', editable: false, align: 'left' },
  { id: 'costs', type: 'dynamic', text: 'Total Biaya: {{TOTAL_BIAYA}}', x: 50, y: 460, width: 694, height: 50, color: '#1a1a1a', fontSize: 18, fontWeight: 'bold', editable: false, align: 'right' },
  { id: 'payment', type: 'dynamic', text: 'Informasi Pembayaran:\n{{INFO_PEMBAYARAN}}', x: 50, y: 530, width: 694, height: 100, color: '#333333', fontSize: 14, fontWeight: 'normal', editable: false, align: 'left' },
];

const InvoiceTemplateEditor = ({ initialData, onSave, onClose }) => {
  ...
  const [modelName, setModelName] = useState('Invoice Default');
      ...
      <div className="h-16 border-b border-border-base bg-bg-surface flex items-center justify-between px-6 shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div>
            <input 
              ...
              placeholder="Nama Model Invoice"
            />
            <p className="text-xs text-text-muted px-1 mt-0.5">Desain posisi dan warna elemen invoice Anda.</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button 
            ...
            className="bg-indigo-base hover:bg-indigo-mid text-white px-6 py-2.5 rounded-xl font-bold text-sm shadow-md transition-all flex items-center gap-2"
          >
          ...
          <div className="shrink-0" style={{ width: `${canvasWidth * 0.8}px`, height: `${canvasHeight * 0.8}px` }}>
            ...
            {(isPreviewMode ? previewElements : elements).map((el) => (
              <Rnd
                ...
                className={`${!isPreviewMode ? 'border-2' : ''} ${selectedId === el.id ? 'border-indigo-base' : 'border-transparent hover:border-gray-200'} transition-colors ${!isPreviewMode ? 'cursor-move' : ''} flex flex-col justify-center`}
        ...
        <div className="w-80 border-l border-border-base bg-bg-surface shrink-0 overflow-y-auto p-6 relative">
          {isPreviewMode && (
            <div className="absolute inset-0 bg-bg-surface/80 backdrop-blur-sm z-20 flex flex-col items-center justify-center p-6 text-center">
              <Icon name="Eye" size={32} className="text-indigo-base mb-4" />
          ...
          {selectedElement ? (
            <div className="space-y-6">
              <div className="p-3 bg-indigo-soft/30 rounded-xl border border-indigo-100">
                <span className="text-xs font-bold text-indigo-700 uppercase tracking-wider block mb-1">Terpilih:</span>
              ...
              {(selectedElement.type === 'text' || selectedElement.type === 'ai_space') && selectedElement.editable && (
                <div>
                  ...
                  <textarea 
                    ...
                    className="w-full p-3 bg-bg-page border border-border-base rounded-xl text-sm focus:ring-2 focus:ring-indigo-base/30 focus:border-indigo-base outline-none resize-none"
                    rows={2}
                  />
                </div>
              )}
                ...
                <>
                  <div>
                    ...
                    <select
                      ...
                      className="w-full p-2.5 bg-bg-page border border-border-base rounded-xl text-sm focus:ring-2 focus:ring-indigo-base/30 outline-none"
                    >
                  ...
                  <div>
                    <label className="block text-sm font-bold text-text-heading mb-2 flex justify-between">
                      ...
                      <span className="text-indigo-base">{selectedElement.borderRadius || 0}px</span>
                    </label>
                    <input
              ...
              <div>
                <label className="block text-sm font-bold text-text-heading mb-2 flex justify-between">
                  ...
                  <span className="text-indigo-base">{selectedElement.rotation || 0}°</span>
                </label>
                <input 
                  type="range" 
                  min="0" 
              ...
              <div>
                <label className="block text-sm font-bold text-text-heading mb-2 flex justify-between">
                  ...
                  <span className="text-indigo-base">{selectedElement.opacity ?? 100}%</span>
                </label>
                <input 
                  type="range" 
                  min="0" 
                ...
                <div className="flex gap-2">
                  {['left', 'center', 'right'].map(align => (
                    <button
                      ...
                      className={`flex-1 py-2 rounded-lg text-sm font-medium border ${selectedElement.align === align ? 'bg-indigo-soft text-indigo-base border-indigo-200' : 'bg-bg-page text-text-muted border-border-base hover:bg-gray-50'}`}
                    >
              ...
              {!['doc_title', 'invoice_no', 'seller_id', 'dates', 'buyer_id', 'packages', 'costs', 'payment'].includes(selectedElement.id) && (
            ...
            <div className="space-y-6">
              ...
              <div className="pt-4 border-t border-border-base">
                ...
                <select
                  ...
                  className="w-full p-2.5 bg-bg-page border border-border-base rounded-xl text-sm focus:ring-2 focus:ring-indigo-base/30 outline-none mb-3"
                >
                ...
                <div className="flex gap-2">
                  <div className="flex-1">
                    ...
                    <input 
                      ...
                      className="w-full p-2 bg-bg-page border border-border-base rounded-lg text-sm focus:ring-2 focus:ring-indigo-base/30 outline-none"
                    />
                  </div>
                  <div className="flex-1">
                ...
                {(customW !== canvasWidth || customH !== canvasHeight) && (
                  <button 
                    ...
                    className="mt-3 w-full py-2 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 text-indigo-700 font-bold rounded-lg text-xs transition-colors shadow-sm"
                  >
              ...
              <div className="pt-4 border-t border-border-base">
                ...
                {uploadedImages.length === 0 ? (
                  <div className="text-center text-text-muted bg-bg-page rounded-xl border border-dashed border-border-base p-4">
                    ...
                    <p className="text-xs">Belum ada gambar. Upload gambar untuk digunakan di invoice.</p>
                  </div>
                ) : (
                  <div className="grid grid-cols-3 gap-2">
                    {uploadedImages.map(img => (
                      <div key={img.filename} className="group relative aspect-square rounded-lg overflow-hidden border border-border-base hover:border-indigo-base transition-all cursor-pointer bg-bg-page">
                        <img
                        ...
                        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-all flex items-center justify-center opacity-0 group-hover:opacity-100">
                          <button
                            ...
                            className="bg-white/90 text-indigo-700 p-1.5 rounded-lg mr-1 hover:bg-white transition-colors shadow-sm" title="Tambah ke canvas"
                          >
