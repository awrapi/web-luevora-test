  ...
  return (
    <div className="relative min-h-full overflow-x-hidden" style={{ background: '#f5f6fa' }}>
      ...
      <div className="relative z-10 px-4 sm:px-6 lg:px-8 py-6 max-w-[1200px] mx-auto">
        ...
        {/* ── Comprehensive Lead Trends Row ── */}
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 mb-6 animate-[fadeSlideUp_0.58s_ease-out]" style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
          <h2 className="text-[14px] font-bold text-gray-800 mb-4 border-b border-gray-100 pb-2">Tren Leads: Masuk, Ghosting, Cancel Order & Repeat Order</h2>
          <div className="h-[280px]">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={leadTrends} margin={{ top: 5, right: 10, left: 10, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                <XAxis dataKey="date" tick={{ fontSize: 10, fill: '#94a3b8' }} axisLine={false} tickLine={false} />
                <YAxis tick={{ fontSize: 10, fill: '#94a3b8' }} axisLine={false} tickLine={false} />
                <Tooltip content={<CustomTooltip />} />
                <Legend wrapperStyle={{ fontSize: '11px', paddingTop: '10px' }} />
                <Bar dataKey="incoming" name="Leads Masuk" fill="#34d399" barSize={12} radius={[4, 4, 0, 0]} />
                <Line type="monotone" dataKey="ghosting" name="Leads Ghosting" stroke="#f87171" strokeWidth={3} dot={{ r: 4, fill: '#f87171' }} activeDot={{ r: 6 }} />
                <Line type="monotone" dataKey="cancelOrder" name="Cancel Order" stroke="#fbbf24" strokeWidth={3} dot={{ r: 4, fill: '#fbbf24' }} activeDot={{ r: 6 }} />
                <Line type="monotone" dataKey="repeatOrder" name="Repeat Order" stroke="#a78bfa" strokeWidth={3} dot={{ r: 4, fill: '#a78bfa' }} activeDot={{ r: 6 }} />
              </ComposedChart>
            </ResponsiveContainer>
          </div>
        </div>
