...
import RentalRequestWidget from '@/components/rental/RentalRequestWidget';
import Icon from '@/components/shared/Icon';
import api from '@/services/api';

/**
 * RentalDashboard Page
...
const RentalDashboard = () => {
  ...
  // Mock requests data for now
  const mockRequests = [
    { assetName: 'Tenda Dome 4P', customerName: 'Andi Pratama', duration: '3 Hari', status: 'Pending' },
    { assetName: 'Carrier 60L', customerName: 'Siska Putri', duration: '2 Hari', status: 'Lunas' },
    { assetName: 'Sleeping Bag', customerName: 'Rian Hidayat', duration: '3 Hari', status: 'Lunas' },
  ];
    ...
    <div className="p-3 sm:p-6 max-w-[1600px] mx-auto">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-8 gap-3">
        <div className="min-w-0">
          <h2 className="text-xl sm:text-2xl font-display font-bold text-text-heading mb-1">Rental Control Center</h2>
          <p className="text-text-muted text-xs sm:text-sm">Monitor leads, aset, dan performa AI Agent Anda.</p>
        </div>
        ...
        <div className="bg-bg-surface border border-border-base p-4 rounded-xl shadow-xs">
          <div className="flex items-center gap-4">
            <div>
              ...
              <div className={`text-[11px] font-bold ${autoFollowUp ? 'text-green-600' : 'text-text-muted'}`}>{autoFollowUp ? 'SISTEM AKTIF' : 'SISTEM NONAKTIF'}</div>
            </div>
            <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
        <div className="lg:col-span-2">
          <RentalRequestWidget requests={mockRequests} />
        </div>
          ...
          <div className="p-5">
            <div className="bg-indigo-soft/50 border border-indigo-border p-5 rounded-xl">
              ...
              <p className="text-xs text-text-body leading-relaxed mb-4">
                Ada {stats.find((s) => s.label === 'Ghosting')?.value || 0} leads "Ghosting" yang bisa dijangkau kembali hari ini. Saran: Gunakan promo{' '}
                <span className="font-bold">"Rental Akhir Pekan"</span> untuk memicu respon.
              </p>
